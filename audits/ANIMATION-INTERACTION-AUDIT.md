...
| P6 | Ambient & hero — **SHIPPED** | lighting coalescer (`0f45951`: motion/light.js → `--lx/--ly`, six `.lit` hosts on ONE viewport-fixed radial; gates = pointer:fine, reduced-motion-never-attaches, hidden-hosts-spend-nothing), particle engine (`3b50809`: ONE pooled canvas, hard cap 240, born on demand, removed at decay; honest counted declines), consumers (`assemble` on the gate's hidden→shown edge, send burst, trash-only dust — ordering pins, `motion-fx-consumers`), timetable camera takeover (push/pop pinned to the workspace's sole hidden-bit writers), empty-state breath (finite-by-pin: 4 breaths, base-pose-exact rest) | `motion-light` 7 pins · `motion-particles` 8 pins · `motion-fx-consumers` 3 pins · `motion-workspace` 3 pins — all green |
| P7 | Final audit pass — **DONE** | Live CDP walk, 9/9: light tracks + sheen is fixed-attachment/click-transparent; camera interpolates (sampled scales) and returns EXACT rest after a mid-flight interrupted reversal; timetable open=recede/Esc=rest; send burst + trash dust birth the canvas then it LEAVES; breath = 3.4s × 4; reduced-motion run: light parked at :root pose, no canvas ever, breath squashed to 0.001s. The walk caught the one bug every node pin missed (P6e fix cycle 2: an orphaned comment tail ate the breath declaration under Blink while jsdom parsed on — the doctrinal point of live walks, proven). 218/218 local contracts green (motion ×10 files, accessibility pins, package, layout-contract). Integration = CI. | — |
