...
| `ANIMATION-INTERACTION-AUDIT.md` | The motion overhaul's steering document: the measured census of today's animation stock (31 keyframes, ~50 default-ease transitions, zero physics), the surface→tier map, the binding spring/depth/lighting vocabulary, and the milestone plan (P2 motion core → P7 final audit) under the standing constraints (reduced-motion inviolable, state-never-waits-for-pixels, app.js frozen). |
