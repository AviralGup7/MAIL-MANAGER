...
| R-A5 | **FIXED `7432b55`** | icon rail (≤860) | Compose becomes a 46px circle; footer actions vanish whole. Follow-up probe (2026-08-13, 860px + 480px, palette queried via the real Ctrl+K path) narrowed this row's claim: Activity and Gmail DID have palette routes (Gmail has Esc too) — the true dead ends were **Timetable and Sign out** (no control, no palette row). Fix: `#btn-side-more`, a footer kebab revealed only inside the 860px block, delegating to the real hidden buttons via `.click()` (never a second implementation); badge count crosses as trailing text. Re-verified at 900/860/480: hidden at 900, centred + clamped + focus-inside at ≤860, delegation dispatches exactly one click on the target button. |
| R-A6 | **VERIFIED-DEFERRED (2026-08-13 probe)** | height | Measured at 1100×{860,560,480,420}: iframe body = 473/185/105/45px ≈ 24/9.5/5.4/2.3 lines. The estimate was exact. But: nothing clips, nothing overlaps, every action stays reachable, body scrolls — degraded, not unusable, and 420px-tall windows are an edge case. Compact reading header stays an opportunity; per the roadmap it is **not** done now. One height rule (topbar 61→49 at ≤700px) confirmed firing. |
| R-A7 | **VERIFIED-CLOSED (2026-08-13 probe)** | toolbar ≤720 | The audit's worry was teachability; the 720px probe shows the teaching channel survives *because* the labels leave: every verb keeps icon + shortcut-carrying tooltip ("Refresh (r)", "Saved views", "Keyboard shortcuts (?)"), icon-only buttons keep aria-labels. Nothing disappears, nothing is unnamed; an overflow menu would duplicate the visible. Future idea (not built): first-run coach could name the icons once at ≤720. |
