...
3. **Load unpacked** → select `/home/user/mail-manager`
4. Open Gmail. Press **Alt+Shift+M**.

### The one thing I most need to know

On `chrome://extensions`, find this extension and click the blue
**“service worker”** link. That opens DevTools for the background script.
Filter the console by `BMM`.
...
- **Yes** → the background script is alive. Everything else is debuggable.
- **No** → nothing works, and that one fact tells me where to start.
