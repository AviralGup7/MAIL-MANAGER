...
src/
  ...
  background/index.js       Service worker: 24 verbs, message router, snooze wake.
  ...
  app/shortcuts.js          The 23 shortcuts, as data. The `?` overlay renders it.
...
test/                       784 tests. `npm test` · `npm run test:ci` (fails on skips)
...
**784 tests pass, 0 skipped.** 129 of them boot the real `app.html` in a real
