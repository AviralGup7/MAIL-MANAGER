...
src/
  ...
  background/index.js       Service worker: 26 verbs, message router, snooze wake.
  background/auth.js        Implicit flow + silent renewal + session epoch. No secret.
  background/gmail.js       REST + multipart batch + MIME extraction + headerMap.
  background/sync.js        Page sync and History API deltas.
  app/app.js                Render loop, triage, keyboard, mailbox switching.
  app/app.css               3 durations, 2 easings, layered STRUCTURE→DEPTH.
  app/store.js              Incremental indexes, batched notification, 2000 cap.
  app/cache.js              Persisted headers; the warm start.
  app/mailboxes.js          The 7 system mailboxes and their per-view actions.
  app/rules.js              Category mute, auto-archive, classifier corrections.
  app/settings.js           One schema, one default per key, coerced on read.
  app/snooze.js             Wake presets, including "day before the deadline".
  app/selection.js          Multi-select with Gmail/Finder shift-range semantics.
  app/contacts.js           Autocomplete built from mail already in the store.
  app/draft-store.js        Debounced autosave + crash recovery.
  app/shortcuts.js          The 24 shortcuts, as data. The `?` overlay renders it.
...
test/                       821 tests. `npm test` · `npm run test:ci` (fails on skips)
...
**821 tests pass, 0 skipped.** 139 of them boot the real `app.html` in a real
