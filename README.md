...
They are retracted in full in `docs/CLASSIFIER-CORRECTION.md`. The short
version:
...
```
manifest.json               MV3 package entry point; least-privilege permissions.
...
src/
  styles/                   THE stylesheet as 33 numbered volumes (was app.css);
  ...
  app/
    ...
    system/                 settings, themes, cache, audience, backup,
                            fallback, deep-links and view/storage registries
    ...
    compose/                compose, autocomplete, templates and draft storage
...
test/                       1,997 declared tests. `npm test` · `npm run test:ci` (fails on skips)
...
**1,997+ declared tests pass, 0 skipped.** Many of them boot the real `app.html` in a real
