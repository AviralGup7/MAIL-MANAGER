...
import { wireAutocomplete, refreshContacts, _resetContacts } from './autocomplete.js';

/*
 * RE-EXPORTED, NOT MOVED AWAY.
 *
 * The radar and the autocomplete now live in their own files. They are
 * re-exported here so every existing importer keeps working -- this split is
 * about where code LIVES, and turning it into a rename of five public
 * functions as well would make one mechanical change into a risky one.
 */
...
export function _resetFeatureState() {
  ...
  composeMeta = {};
  draftSaver = null;
  // The address book moved to autocomplete.js; reset it where it lives.
  _resetContacts();
  pendingFiles = [];
}
 ...
 *
 * Lived below the autocomplete section, so splitting on section boundaries
 * carried it into autocomplete.js -- where it had one caller and compose had
 * eight. Proximity in a file is not ownership.
 */
